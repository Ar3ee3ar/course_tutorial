  ...
  padding: 0;
    ...
    <script>
      function search(){
            ...
            <?php
                    echo 'var url = "search_his.php?search="+search;';
            ?>
        ...
        function confirmCancel(ID_sub){
            let ans = confirm("ต้องการยกเลิกคำสั่งซื้อ"+ID_sub);
            if(ans==true)
                document.location = "cancel.php?id="+ID_sub;
        }
...
<?php
  ...
  JOIN status
  ON history.ID_status = status.ID_status WHERE m_username="'.$m_username.'"ORDER BY ID_history DESC';
